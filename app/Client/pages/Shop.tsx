      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="flex flex-col lg:flex-row gap-8">
              ...
              <div className="space-y-2">
                {categories.map((cat) => (
                  <button
                    ...
                    className={`w-full text-left px-4 py-2 text-sm transition-colors ${selectedCategory === cat ? "text-gold-500 border-l border-gold-500 pl-3" : "text-zinc-500 hover:text-white"}`}
                  >
                    {cat}
                  </button>
                ))}
          ...
          <div className="flex-1">
            ...
            <div className="flex justify-between items-center mb-8 border-b border-zinc-800 pb-4">
              ...
              <div className="flex items-center gap-4">
                <div className="hidden md:flex gap-2 text-zinc-600">
                  <button onClick={() => setGridCols(2)} className={gridCols === 2 ? "text-gold-500" : "hover:text-white"}><LayoutGrid className="w-5 h-5"/></button>
                  <button onClick={() => setGridCols(3)} className={gridCols === 3 ? "text-gold-500" : "hover:text-white"}><Grid3x3 className="w-5 h-5"/></button>
                </div>
                <div className="relative">
                   <select 
                    value={sortBy} 
                    onChange={(e) => setSortBy(e.target.value)}
                    className="bg-zinc-900 border border-zinc-800 text-white text-sm px-4 py-2 pr-8 appearance-none focus:border-gold-500 outline-none"
                   >
            ...
            {loading ? (
              <div className="flex justify-center py-20"><div className="w-8 h-8 border-2 border-gold-500 border-t-transparent rounded-full animate-spin"></div></div>
            ) : (
