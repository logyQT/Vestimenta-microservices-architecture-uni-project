...
export function ProductDetail() {
  ...
  if (loading || !product) return <div className="min-h-screen bg-dark-900 pt-32 flex justify-center"><div className="animate-spin w-8 h-8 border-2 border-gold-500 border-t-transparent rounded-full"></div></div>;
           ...
           <div className="space-y-4">
              ...
              {product.images && (
                <div className="grid grid-cols-4 gap-4">
                  <div 
                    className={`aspect-square cursor-pointer border ${mainImage === product.image ? 'border-gold-500' : 'border-transparent'}`}
                  ...
                  {product.images.map((img, idx) => (
                    <div 
                        key={idx} 
                        className={`aspect-square cursor-pointer border ${mainImage === img ? 'border-gold-500' : 'border-transparent'}`}
           ...
           <div className="flex flex-col h-full">
                 ...
                 <div className="flex flex-wrap gap-3">
                    {product.sizes?.map(size => (
                       <button
                          ...
                          className={`w-12 h-12 flex items-center justify-center border text-sm transition-all ${selectedSize === size ? 'bg-gold-500 border-gold-500 text-black' : 'border-zinc-700 text-zinc-400 hover:border-white'}`}
                       >
                          {size}
                       </button>
                    ))}
              ...
              <div className="mt-auto">
                 <button 
                    ...
                    className="w-full bg-white text-black py-4 font-medium uppercase tracking-widest hover:bg-gold-500 transition-colors duration-300"
                 >
                    Add to Bag
                 </button>
