      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-12 border-b border-zinc-800 pb-8">
          ...
          <button
            ...
            className="text-red-500 border border-zinc-800 px-6 py-2 hover:bg-red-500/10 transition-colors text-sm"
          >
            Sign Out
          </button>
        </div>
        ...
        <div className="grid md:grid-cols-4 gap-8">
          ...
          <div className="space-y-2">
            <button onClick={() => setActiveTab("orders")} className={`w-full flex items-center gap-3 px-4 py-3 text-sm transition-colors ${activeTab === "orders" ? "bg-gold-500 text-black" : "bg-zinc-900 text-zinc-400 hover:bg-zinc-800"}`}>
              <Package className="w-4 h-4" /> My Orders
            </button>
            <button onClick={() => setActiveTab("details")} className={`w-full flex items-center gap-3 px-4 py-3 text-sm transition-colors ${activeTab === "details" ? "bg-gold-500 text-black" : "bg-zinc-900 text-zinc-400 hover:bg-zinc-800"}`}>
                  ...
                  <div key={order.id} className="bg-zinc-900/50 border border-zinc-800 p-6">
                    <div className="flex flex-col md:flex-row justify-between md:items-center mb-4 border-b border-zinc-800 pb-4 gap-4">
                      ...
                      <div className="flex items-center gap-4">
                        <span className={`text-xs px-3 py-1 uppercase tracking-wider ${order.status === "Delivered" ? "bg-green-900/30 text-green-400" : "bg-yellow-900/30 text-yellow-500"}`}>{order.status}</span>
